...
- **Who reads an artifact is a profile setting, not a habit.** `artifacts: {language, register, audience}` — `board.py writing` prints the resulting brief and every skill asks it before writing prose. A wiki page answers *what is this and how does it work*; a work item answers *what exactly do we build*, and a fact only one feature cares about does not belong on a wiki page (IDE-139, IDE-140).
...
- **A phase the board cannot express as a status is carried by a tag.** The profile's `phases` map takes `{"tag": "idp:in-design"}` where a board has no status to map — see [IDE-125](https://linear.app/krukov-idea-hub/issue/IDE-125/work-item-ide-92-phase-positions-carried-by-tags-where-a-board-has-no). A bare string still means a status. Tags in the `idp:` namespace are mutually exclusive, a tag outranks the status it sits on, and the swap is one write. The price: a phase carried by a tag does not move a card across a board column, so the board is no longer readable by eye — cheaper than the comment fallback, dearer than real statuses. Both adapters implement it ([IDE-126](https://linear.app/krukov-idea-hub/issue/IDE-126/work-item-ide-92-azure-devops-phase-positions-carried-by-tags) did the Azure DevOps half, where the swap is one revision of `System.Tags` and the correlation id survives it). **What is still unconfirmed is the premise:** that an Azure DevOps `System.Tags` change appears in the revision history with `System.ChangedBy`. The claim protocol reads the actor from history; if tags are not attributed there, they cannot carry a phase for parallel agents on that board.
- **The nine board statuses exist** — `Ready for Design`, `In Design`, `Design Review`, `Ready for Planning`, `In Planning`, `Ready for Development`, `In Development`, `Blocked - Needs Design`, `PR Review` — created by hand in Settings → Teams → IdeaHub → Workflow, because Linear has no API for creating them. **That hand step is the gap**: a foreign team either creates the same nine or maps its existing ones in the profile's phase table. Where a status cannot exist at all — every Azure DevOps process, where states belong to the work item type — the position is carried by an `idp:` tag, which is the entry above and is what both adapters now default to on that board. `null` is the floor below that: the phase is recorded as a comment and the claim protocol degrades to comment order, at the cost of a board no longer readable by eye.
...
- **This file is a hand-maintained summary, and it has drifted before.** Two entries here described a repository that no longer existed, and a session that read them cold went hunting a credential bug that had already been fixed ([IDE-131](https://linear.app/krukov-idea-hub/issue/IDE-131/claudemd-known-gaps-lists-two-gaps-that-are-closed)). `board.py memory check` now warns when this file is older than the last commit touching `scripts/`, `skills/`, `schemas/`, `templates/`, `lint/` or `registry/`. It is a warning and not a refusal, because most of those commits change nothing this file claims — which is also why the drift is invisible one commit at a time. When it warns, read the claims against the commits it lists.

Closed since the last revision, kept here only so a reader who remembers them stops looking: **`read_token` is board-aware** — `scripts/board.py` carries a `CREDENTIALS` map keyed by board, `azure-devops` resolves to `AZURE_DEVOPS_EXT_PAT`, and IDE-130 gave that board a `token_path` too. **The content validator runs before publication** — `skills/feature-discovery/publish_linear.py` calls `validate_text(...)` on the package before it ever reaches the board, which was IDE-111. Per-agent keys shipped with IDE-100 — the profile's `agents` map holds one token path per agent, and the claim protocol reads them apart in history. IDE-68 §8.1 is no longer provisional; it was reconciled against the approved IDE-71 contract, and a feature is created in `design · ready` — `Ready for Design` on Linear — rather than `Todo` with a `stage:*` label, because swapping a label leaves no history for the claim protocol to read.
